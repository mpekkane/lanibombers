"""
PlayerSetupView — arcade.View wrapper around the player_setup logic.

All UI logic is preserved from player_setup.py. The key differences:
- Extends arcade.View instead of arcade.Window
- Initialization happens in on_show_view so the view can be shown multiple times
- self.width/self.height replaced with self.window.width/self.window.height
- arcade.close_window() replaced with show_view(MainMenuView())
"""
...
class PlayerSetupView(arcade.View):
    """Player setup GUI as an arcade.View."""
    ...
    def _init_sprite_lists(self):
        ...
        map_x = self.window.width - map_width - 10  # 10 pixels from right edge
        map_y = self.window.height - 30 * self.zoom  # 30 sprite pixels from top
